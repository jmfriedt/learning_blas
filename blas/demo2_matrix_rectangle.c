...
// CblasRowMajor ou CblasColMajor ? row-major (C) or column-major (Fortran) data ordering
  cblas_sgemm(CblasRowMajor, CblasTrans, CblasNoTrans, nlag , nlag, nobs, alpha, host_val, nlag, host_val, nlag, beta, host_res, nlag ); 
  affiche_matrice(host_res,nlag,nlag);
  cblas_sgemm(CblasRowMajor, CblasNoTrans, CblasTrans, nobs , nobs, nlag, alpha, host_val, nobs, host_val, nobs, beta, host_res, nobs ); 
